...
import type { ReportInfo, ReportRunInfo } from '@/types/analytics'
...
/** 报告运行历史：运行列表（状态/耗时/错误）+ 选中运行的结果快照（表格 + 图表）。 */
export function ReportRunsDialog({
  report,
  onClose,
}: {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      // 追加下一页；去重避免后端未按 offset 分页时重复追加；截取 limit 条，余量仅作 hasMore 信号
      const fresh = next.filter((r) => !runs.some((x) => x.id === r.id))
      setRuns((prev) => [...(prev ?? []), ...fresh.slice(0, 20)])
  ...
  return (
    <Dialog open={report !== null} onOpenChange={(v) => (!v ? onClose() : undefined)}>
